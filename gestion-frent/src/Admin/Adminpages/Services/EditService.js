...
export default function UpdateServece() {
  let { id } = useParams();
const [data,setData] = useState([])
  ...
  useEffect(() => {
    axios
      .get(`http://127.0.0.1:8000/api/services/show/${id}`)
      .then(({ data }) => {
        const { nom_service } = data.Services;
  ...
  const Update = (e) => {
    ...
    axios
      .post("http://127.0.0.1:8000/api/services/update/" + id, formdata)
      .then((data) => {
        swal("success!", "modefier success", "success");
        window.location.pathname = "/listServices";
      })
      .catch(({ response }) => {
